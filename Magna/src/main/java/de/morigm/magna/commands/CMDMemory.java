        ...
        if (checkPermission(com, "memory")) {
            if (args.length >= 1) {
                if (args[0].equalsIgnoreCase("GIGABYTE") || args[0].equalsIgnoreCase("GB")) {
                    com.sendPlainMessage(Chat.prefix + translate("cmd.memory.used") + ":"
                            + getMemoryManager().getUsedMemory(MemoryType.GIGABYTE) + "GB");
                    com.sendPlainMessage(Chat.prefix + translate("cmd.memory.free") + ":"
                            + getMemoryManager().getFreeMemory(MemoryType.GIGABYTE) + "GB");
                    com.sendPlainMessage(Chat.prefix + translate("cmd.memory.total") + ":"
                    ...
                    com.sendPlainMessage(Chat.prefix + Slash(com) + getCommand() + " <GIGABYTE,MEGABYTE,KILOBYTE>");
            } else {
                com.sendPlainMessage(Chat.prefix + translate("cmd.memory.used") + ":"
                        + getMemoryManager().getUsedMemory(MemoryType.MEGABYTE) + "MB");
                com.sendPlainMessage(Chat.prefix + translate("cmd.memory.free") + ":"
